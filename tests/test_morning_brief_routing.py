...
class MorningBriefRoutingTests(unittest.TestCase):
    ...
    def test_splitting_and_allowed_mentions_apply_to_each_destination(self):
        ...
        self.assertTrue(all(len(payload["content"]) <= 12 for payload in payloads))
        self.assertTrue(all("earnings" not in payload["content"] for payload in payloads[:2]))
        self.assertTrue(all("morning" not in payload["content"] for payload in payloads[2:]))
        sleep.assert_called_once_with(10)
